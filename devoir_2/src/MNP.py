...
def d2S_dr2(spline, r, t):
    return spline.ev(t, r, dx=0, dy=2)

def dS_dr(spline, r, t):
    return spline.ev(t, r, dx=0, dy=1)

def dS_dt(spline, r, t):
    return spline.ev(t, r, dx=1, dy=0)

def source_term(spline, r, t, D, k):
    # Avoid division by zero for r=0
    r = max(r, 1e-6)

    # Compute necessary derivatives of S
    C_t = dS_dt(spline, r, t)
    C_r = dS_dr(spline, r, t)
    C_rr = d2S_dr2(spline, r, t)
    
    # Evaluate the spline itself for the sink term
    C = spline(t, r)[0, 0]

    # Substitute into the PDE and solve for the source term f, including the sink term -kS
    S = C_t - D * (C_rr + (1/r) * C_r) + k * C
    return S
...
while not converged and iteration < max_iterations:
    ...
    for i in range(1, I-1):
        r_i = i * d_r
        rhs[i] += source_term(spline, r_i, current_time, D, k) * d_t
