...
plt.ylabel(r'$\delta_{model}=C_{D_{sim}}-C_{D_{exp}}$', fontsize=12)
plt.ylim(-0.10, 0.20)
plt.xlim(-5, 20)
...
output_data = pd.DataFrame({
    ...
    'input_un': input_un,
    'exp_un': exp_un,
    'ci_y': np.divide(ci_y,2)
})

# Write the DataFrame to a .txt file
output_file_path = '../results/CD_validation_data.txt'  # Define the file path
output_data.to_csv(output_file_path, sep='\t', index=False)  # Save the file without the index column
