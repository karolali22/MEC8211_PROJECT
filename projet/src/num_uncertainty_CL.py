...
def extract_values(filename):
    with open(filename, 'r') as file:
        ...
        cl_match = re.search(r'CL = ([\d.-]+e[\d+-]+)', content)
...
def gather_data_and_compute(folder_path):
    ...
    for folder in folders:
        ...
        for filename in os.listdir(dir_path):
            ...
            aoa, cl = extract_values(full_path)
            if aoa and cl is not None:
                if aoa not in data:
                    data[aoa] = {}
                data[aoa][folder] = cl
    ...
    for aoa, values in data.items():
        if all(folder in values for folder in folders):
            ...
            if p_hat is not None:
                ...
                results[aoa] = {
                    'p_hat': p_hat, 'GCI': gci,
                    'CL_65': values['../data/convergence_results/65x65'],
                    'CL_129': values['../data/convergence_results/129x129'],
                    'CL_257': values['../data/convergence_results/257x257']
                }
            else:
...
def main():
    ...
    with open('../data/CL/num_uncertainty_CL.txt', 'w') as file:
